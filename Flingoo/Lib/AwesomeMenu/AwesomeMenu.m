...
- (void)AwesomeMenuItemTouchesBegan:(AwesomeMenuItem *)item
{
    if (item == _startButton) 
    {
        ...
        self.expanding = !self.isExpanding;
        if ([item.lableArray count]==5)
        {
            self.lblTitleChat=(UILabel *)[item.lableArray objectAtIndex:0];
            self.lblTitleGift=(UILabel *)[item.lableArray objectAtIndex:1];
            self.lblTitleFriend=(UILabel *)[item.lableArray objectAtIndex:2];
            self.lblTitleFav=(UILabel *)[item.lableArray objectAtIndex:3];
            self.lblTitleRepo=(UILabel *)[item.lableArray objectAtIndex:4];
        }
...
- (void)AwesomeMenuItemTouchesEnd:(AwesomeMenuItem *)item
{
    ...
    if(self.delegate && [self.delegate respondsToSelector:@selector(awesomeMenuWillFinishAnimationClose:)]){
        [self.delegate awesomeMenuWillFinishAnimationClose:self];
    }
...
//    }];
    ...
    if ([_delegate respondsToSelector:@selector(awesomeMenu:didSelectIndex:)])
    {
        ...
        self.lblTitleChat.hidden=YES;
        self.lblTitleGift.hidden=YES;
        self.lblTitleFriend.hidden=YES;
        self.lblTitleFav.hidden=YES;
        self.lblTitleRepo.hidden=YES;
    }
}
...
- (void)setExpanding:(BOOL)expanding
{
	if (expanding) {
		[self _setMenu];
        //hemalasankas
        if(self.delegate && [self.delegate respondsToSelector:@selector(awesomeMenuWillAnimationOpen:)]){
            [self.delegate awesomeMenuWillAnimationOpen:self];
        }
	}
...
- (void)_expand
{
    ...
    fadeAnimation.beginTime = CACurrentMediaTime() + 0.5f;
    fadeAnimation.additive = NO;
    fadeAnimation.removedOnCompletion = NO;
    fadeAnimation.duration = 0.30;
    fadeAnimation.fillMode = kCAFillModeBoth;
    [item.titleLab.layer addAnimation:fadeAnimation forKey:@"opacity"];
...
//            self.lblTitleChat.hidden=NO;
//            self.lblTitleGift.hidden=NO;
//            self.lblTitleFriend.hidden=NO;
//            self.lblTitleFav.hidden=NO;
//            self.lblTitleRepo.hidden=NO;
