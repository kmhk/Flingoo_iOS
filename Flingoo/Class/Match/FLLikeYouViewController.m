...
@interface FLLikeYouViewController ()<UITableViewDelegate, UITableViewDataSource, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UIAlertViewDelegate>
...
@property(nonatomic, assign) LikeYouLockType lockType;

@end

@implementation FLLikeYouViewController
...
-(id) initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil{
    ...
    if(self){
        self.tabBarItem.image = [UIImage imageNamed:@"match_LikeYou.PNG"];
        self.title = @"Like you!";
...
- (void)viewDidLoad{
    ...
    self.navigationItem.title = @"Like you";
    ...
    //set your lock type
    if([[NSUserDefaults standardUserDefaults] objectForKey:@"WHO_LIKES_YOU"]){
        self.lockType = kLikeYouLockTypeUnlocked;
    }else{
        self.lockType = kLikeYouLockTypeLocked;
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //set your data here,
    if(self.lockType==kLikeYouLockTypeLocked){
        
//        [cell setOnline:[self randomBOOL]];
         [cell setOnline:[profileObj.is_online boolValue]];
        [cell.profilePictureView setImage:[UIImage imageNamed:@"dummyProPicMale.png"]];
        cell.profileNameLabel.text = @"Anonymous";
        cell.subtitleLine1.text = @"";
        cell.subtitleLine2.text = @"";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if(self.lockType == kLikeYouLockTypeLocked){
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"paymeny_required", nil) message:NSLocalizedString(@"pay_credits_see_liked_you", nil) delegate:self cancelButtonTitle:NSLocalizedString(@"no", nil) otherButtonTitles:NSLocalizedString(@"yes", nil), nil];
        [alert show];
    }else{
        //load
        HUD=[[MBProgressHUD alloc] initWithView:self.view];
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex{
    if (buttonIndex==1) {
        ...
        [[NSUserDefaults standardUserDefaults] setObject:[NSNumber numberWithBool:YES] forKey:@"WHO_LIKES_YOU"];
        //load
        self.lockType = kLikeYouLockTypeUnlocked;
        [self.personCollectionView reloadData];
        [self.personTableView reloadData];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(self.lockType==kLikeYouLockTypeLocked){
        [cell setOnline:[profileObj.is_online boolValue]];
        [cell.profilePicImageView setImage:[UIImage imageNamed:@"dummyProPicMale.png"]];
        cell.profileNameLabel.text = @"Anonymous";
