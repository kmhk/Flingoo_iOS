...
@interface FLGroupChatScreenViewController ()<UITextFieldDelegate, UITableViewDataSource, UITableViewDelegate, UIActionSheetDelegate,UINavigationControllerDelegate, UIImagePickerControllerDelegate>
...
@implementation FLGroupChatScreenViewController
...
- (void)viewDidLoad{
    ...
    NSLog(@"BOTTOM_PANEL_HIDDEN_FRAME:%@", NSStringFromCGRect(BOTTOM_PANEL_HIDDEN_FRAME));
...
//    [self scrollToBottom];
    ...
    //<##>
        ...
        [webService userChatListForGroup:self withGrouID:self.currentChatObj.chatObj_id];
}
...
- (void) goBack
{
    if ([self.navigationController.viewControllers[0] isKindOfClass:[FLGroupChatScreenViewController class]]) {//came from othere view
...
-(void)chatChanged:(NSNotification *)notification
{
    ...
    if ([[NSString stringWithFormat:@"%@",chatObj.chatObj_id] isEqualToString: [NSString stringWithFormat:@"%@",self.currentChatObj.chatObj_id]])
    {
...
//        chatMsgObj.message=chatMsgObj.message;
//        chatMsgObj.chatDateTime=dateString;
//        chatMsgObj.username=chatMsgObj.username;
//        chatMsgObj.userID=chatMsgObj.userID;
...
//        
//        if(self.isBottomPanelVisible){
//            [self adjustTableViewToHeight:BOTTOM_PANEL_VISIBLE_FRAME.origin.y];
//            [UIView animateWithDuration:0.3f animations:^{
//                self.keyboardPanel.frame = BOTTOM_PANEL_VISIBLE_FRAME;
//            }];
//        }else{
//            [self adjustTableViewToHeight:BOTTOM_PANEL_HIDDEN_FRAME.origin.y];
//            [UIView animateWithDuration:0.3f animations:^{
//                self.keyboardPanel.frame = BOTTOM_PANEL_HIDDEN_FRAME;
//            }];
//        }
//        [self performSelector:@selector(scrollTableViewToBottom) withObject:nil afterDelay:0.3];
//    }
...
//    FLChat *chatObj=[[FLGlobalSettings sharedInstance].chatArr objectAtIndex:chatUserIndex];
    ...
    FLWebServiceApi *webService=[[FLWebServiceApi alloc] init];
    [webService chatSend:self withMessage:chatMsg.message withReceiverID:self.currentChatObj.chatObj_id withType:MSG_TYPE_GROUP];//current chatObj id
