...
@interface FLFavouritsViewController() <UITableViewDataSource, UITableViewDelegate, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>
...
@implementation FLFavouritsViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.tabBarItem.image = [UIImage imageNamed:@"fav_fav.png"];
        favoriteListArr=[[NSMutableArray alloc] init];
    }
    return self;
}
...
-(void)callReload
{
    ...
    [webService favouriteListing:self];
    //    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    profileObj=(FLOtherProfile *)[favoriteListArr objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
      [self cellSelect:(FLOtherProfile *)[favoriteListArr objectAtIndex:[indexPath row]]];
}
...
-(IBAction) myFansButtonPressed:(id)sender;{

    if(IS_IPAD){
        NSDictionary *dict = @{RemoteAction:kRemoteActionShowMyFans};
        self.communicator(dict);
    }else{
        FLMyFansViewController *myFansViewController = [[FLMyFansViewController alloc] initWithNibName:(IS_IPHONE5)?@"FLMyFansViewController-568h":@"FLMyFansViewController" bundle:nil];
...
{
    [HUD hide:YES];
    favoriteListArr=favoritesObjArr;
    [self.personTableView reloadData];
    [self.personCollectionView reloadData];
